...
RUN  apt-get update && apt-get  install python3-pip  -y && pip3 install --upgrade pip \
      ...
      && python3 -m pip install geoarrow-pyarrow geoarrow-pandas rpy2 rpy2-arrow kerchunk s3fs planetary.computer dask-expr jupyter xstac xpystac tifffile VirtualiZarr pygmt rechunker
